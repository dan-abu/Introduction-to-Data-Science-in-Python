...
def answer_two():
    ...
    #Outer
    dfs = [ScimEn, Energy, GDP]
    task2_df = reduce(lambda left,right: pd.merge(left,right,on=['Country'],how='outer'), dfs)
    years2drop = []
    years = range(1960, 2006)
    for year in years:
        years2drop.append(year)
    task2_df.drop(years2drop, axis=1, inplace=True) 
    task2_df.drop(['Country Code', 'Indicator Name', 'Indicator Code'], axis=1, inplace=True)
    task2_df.columns = task2_df.columns.astype(str)
    task2_df.set_index('Country', inplace=True)
    
    second_ans = task2_df.shape[0] - task1_df.shape[0]
    return second_ans
